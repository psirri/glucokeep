...
class DataLabel:
    """ A class that references one data file and corresponding label file. """
...
def get_files(directory):
    ...
    for file in scandir(directory):
        # use file extensions to determine mission and data format
        if file.is_file and (file.name.endswith('.lbl') or file.name.endswith('.dat')):
            is_rosetta = True
        if file.is_file and file.name.endswith('.iq'):
            is_dawn = True

    if is_rosetta and is_dawn:
        print('error in identifying mission 1')
    elif not is_rosetta and not is_dawn:
        print('error in identifying mission 2')
    elif is_rosetta:
        return get_files_rosetta(directory)
    elif is_dawn:
        return get_files_dawn(directory)
    else:
        print('error in identifying mission 3')
...
def get_files_rosetta(directory):
    ...
    # TODO: compare runtime to mKelley _find_file()
    mission = 'Rosetta'
...
def file_to_numpy_dawn(data_label, use_binary_version=True):
    if use_binary_version:
        ...
        if data_label.file_name == '203VSOC2011358_0330NNNX43LD.2B2':
            return np.load('203VSOC2011358_0330NNNX43LD.2B2.npy')
        elif data_label.file_name == '203VSOC2011358_0330NNNX43RD.2A2':
            return np.load('203VSOC2011358_0330NNNX43RD.2A2.npy')
    else:
        # set the Numpy dtype for all columns
        dt = np.dtype(get_dtypes(data_label))
